import time
from document_loader.vector_db_utils import load_vector_db, validate_vector_db
from document_loader.query_utils import normalize_query
from document_loader.rbac_utils import get_allowed_roles


TOP_K = 5

# 🔁 POLICY TOGGLE
# True  -> Strict RBAC (search full DB, then DENY if no authorized results)
# False -> Soft RBAC (search only allowed docs)
...
def secure_semantic_search(query: str, user_role: str):
    ...
    # ================= SEARCH LOGIC =================
    if STRICT_RBAC_MODE:
        # 🔒 STRICT MODE: search entire DB
        all_results = vectordb.similarity_search_with_score(
            query,
            k=TOP_K
        )
        ...
        # 🔓 SOFT MODE: RBAC applied DURING search
    ...
    print(f"Top-K Retrieved     : {total_results}")
    print(f"Authorized Results  : {len(results)}")
    print(f"Search Latency      : {latency} ms")
   

    # 🚫 STRICT RBAC DENY
    if STRICT_RBAC_MODE and len(results) == 0:
